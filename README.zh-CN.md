...
> [!IMPORTANT]  
> 为了展现最佳视觉效果，你需要一个不打印控制字符的终端。在 Windows 上， conhost.exe 不受支持。最新版的 Windows Terminal 同样也有不兼容问题。我们建议使用 Alacritty (请参照 Assets/ 中的截图)。 在 macOS 上使用默认终端似乎没问题。
> 
...
>
> 如果你使用 Windows 并且一定要使用 Windows Terminal，请安装 `windows-terminal@1.19.10302.0` （我的版本）
>
> 该问题已经在 Terminal.Gui 中修复，但它升级到了 v2，而 SonicLair.Cli 还没有准备好（可能永远不会）。
> 
